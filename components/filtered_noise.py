...
class FilteredNoise(nn.Module):
    ...
    def forward(self, z):
        ...
        zero_phase_ir_bank = torch.fft.irfft(INPUT_FILTER_COEFFICIENT, dim=-1, n=filter_coeff_length * 2 - 1) 
        zero_phase_ir_bank = zero_phase_ir_bank.view(zero_phase_ir_bank.shape[0]*zero_phase_ir_bank.shape[1], zero_phase_ir_bank.shape[2])
        ...
        ZERO_PADED_NOISE = torch.view_as_real(ZERO_PADED_NOISE)
        ...
        FILTERED_NOISE = torch.view_as_complex(FILTERED_NOISE)
        filtered_noise = torch.fft.irfft(FILTERED_NOISE, dim=1).view(batch_num, frame_num, -1) * self.attenuate_gain         
